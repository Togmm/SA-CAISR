...
from model import SASRec_DPO_sequence
from utils import *
...
if __name__ == '__main__':
    ...
    log = open(os.path.join(args.dataset + '_' + args.train_dir, 'log.txt'), 'w')
    ...
    model = SASRec_DPO_sequence(itemnum, args).to(args.device)
    # model_ref = SASRec_DPO(itemnum, args).to(args.device)
    periods = get_periods(args.dataset)
    dataloader = DataLoader_DPO(args.dataset, args.maxlen)
    ...
    for period in periods:
        ...
        data = dataloader.train_loader(period - 1)

        train_sampler = Sampler_DPO_sequence(data, args.maxlen, args.batch_size, True)
        ...
        if args.inference_only:
            args.state_dict_path = '/data/wangxinru-slurm/project/Rec/SASRec/SASRec/TAOBAO_TAOBAO_DPO/period2/epoch=1.ckpt'
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            for step in range(num_batch): # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
                flag, length, seq, pos = train_sampler.sampler()
                flag, length, seq, pos = np.array(flag), np.array(length), np.array(seq), np.array(pos)
                win_logits, lose_logits, new_data_logits, new_data_labels = model(flag, length, seq, pos, max_item)
                win_logits_ref, lose_logits_ref, _, _  = model_ref(flag, length, seq, pos, max_item)
                adam_optimizer.zero_grad()
                diff = win_logits - lose_logits
                diff_ref = win_logits_ref - lose_logits_ref
                loss = - F.logsigmoid(args.beta*(diff - diff_ref))# /labels.sum(axis=-1, keepdims=True)
                # if len(new_data_logits) != 0: loss += ce_loss(new_data_logits, new_data_labels)
                loss.backward()
                adam_optimizer.step()
            ...
            valid_sampler = Sampler(user_valid, args.maxlen, args.test_batch, is_subseq=True)
